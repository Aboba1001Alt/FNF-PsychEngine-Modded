...
class PlayState extends MusicBeatState
{
	...
	public var camModControls:FlxCamera;
	...
	override public function create()
	{
		...
		camModControls = new FlxCamera();
		camHUD.bgColor.alpha = 0;
		camOther.bgColor.alpha = 0;
		camModControls.bgColor.alpha = 0;

		FlxG.cameras.reset(camGame);
		FlxG.cameras.add(camHUD, false);
		FlxG.cameras.add(camOther, false);
		FlxG.cameras.add(camModControls, false);
		...
		super.create();
		if (!ClientPrefs.data.noDataClear) Paths.clearUnusedMemory();
	...
	function generateNotes(?jsondata:SwagSong = null) {
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, daogNoteData, oldNote);
				swagNote.mustPress = gottaHitNote;
				swagNote.sustainLength = songNotes[2];
				swagNote.gfNote = (section.gfSection && (songNotes[1]<4));
				swagNote.noteType = songNotes[3];
				if (ClientPrefs.data.oldSupport && swagNote.noteType != "") swagNote.rgbShader.enabled = false;
				...
				if(floorSus > 0) {
					for (susNote in 0...floorSus+1)
					{
						...
						var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote), daNoteData, daogNoteData, oldNote, true);
						sustainNote.mustPress = gottaHitNote;
						sustainNote.gfNote = (section.gfSection && (songNotes[1]<4));
						sustainNote.noteType = swagNote.noteType;
						if (ClientPrefs.data.oldSupport && sustainNote.noteType != "") sustainNote.rgbShader.enabled = false;
	...
	public function callOnHScript(funcToCall:String, args:Array<Dynamic> = null, ?ignoreStops:Bool = false, exclusions:Array<String> = null, excludeValues:Array<Dynamic> = null):Dynamic {
		...
		for(i in 0...len)
		{
			...
			if(script == null || !script.exists(funcToCall) || exclusions.contains(script.scriptFile))
				continue;
			...
			try
			{
				...
				if(!callValue.succeeded)
				{
					...
					if(e != null)
						FunkinLua.luaTrace('ERROR (${script.scriptFile}: ${callValue.calledFunction}) - ' + e.message.substr(0, e.message.indexOf('\n')), true, false, FlxColor.RED);
				}
				else
				{
